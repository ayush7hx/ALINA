...
module.exports = async (client, track) => {
  ...
  if (client._alinaHandlersLoaded) return;
  client._alinaHandlersLoaded = true;
  ...
  client.manager.on("queueEnd", async (player) => {
    try {
      ...
      const embed = new EmbedBuilder()
        .setAuthor({
          name: "ALINA Music Queue Finished",
          iconURL: client.user.displayAvatarURL(),
        })
        .setColor(client.color)
        .setDescription(
          `> The music queue has now reached its end.\n\n` +
          `> Thank you for vibing with **ALINA**! 💜\n\n` +
          `> Invite ALINA to more servers or join our support server to stay updated:\n\n` +
          `> [Invite ALINA](https://discord.com/oauth2/authorize?client_id=${client.user.id}&permissions=8&integration_type=0&scope=bot)\n` +
          `> [Join Support Server](https://discord.gg/7JaEAAnYzc)`
        )
        .setFooter({
          text: "Made by Ayush7hx",
          iconURL: client.user.displayAvatarURL(),
        })
        .setImage(
